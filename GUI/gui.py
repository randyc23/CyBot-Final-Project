...
from comms import (
    BotConnection,
    PACKET_SCAN, PACKET_POSITION, PACKET_BUMP, PACKET_CLIFF, PACKET_SWEEP_START, PACKET_SWEEP_END,
    SweepStartPacket, SweepEndPacket, ScanPacket, PositionPacket, BumpPacket, CliffPacket
)
...
class MainWindow(QMainWindow):
    ...
    def _handle_packet(self, packet):
        ...
        if packet.type == PACKET_POSITION:
            ...
            # TODO: detect end of sweep — either via a dedicated END packet from
            # the bot, or by detecting a gap in incoming scan packets.

        elif packet.type == PACKET_BUMP:
            self.world.handle_bump(packet.sensor_id)
            self._log(f"BUMP event: {packet.sensor_id}")

        elif packet.type == PACKET_CLIFF:
            self.world.handle_cliff(packet.sensor_id)
            self._log(f"CLIFF event: {packet.sensor_id}")
