...
class WorldMap:
    ...
    def add_scan_reading(self, scan_angle: float, ir: float, ping: float):
        ...
        MAX_RANGE = 70.0  # cm — readings beyond this are treated as no-detection

        if ping >= MAX_RANGE or ir < 1000:
            return
    ...
    def handle_cliff2(self, sensor_id: str, type):
        ...
        if(sensor_id == "F"):
            cliff_x, cliff_y = polar_to_global(
                self.bot.x, self.bot.y, self.bot.angle,
                0, 16
            )
        elif(sensor_id == "FL"):
        ...
        if(type == "BORDER"):
            if(cliff_x and cliff_y):
                self.objects.append(
                    WorldObject(
                        center_x=cliff_x,
                        center_y=cliff_y,
                        obj_type=ObjectType.BORDER,
