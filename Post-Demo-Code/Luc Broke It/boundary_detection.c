...
boundary_status_t avoid_hole_boundary(oi_t *sensor_data){
    ...
    int l_bound = cliffLeft > 2600; // change back to 2600
    ...
    if(any_bound){ //Boundary detection
        ...
        if(fl_bound && fr_bound){
            uart_sendStr("E\tF\r\n");
            status = BOUNDARY_FRONT;
        }
        else if(fl_bound) {
            uart_sendStr("E\tFL\r\n");
            status = BOUNDARY_FRONT_LEFT;
        }
        else if(fr_bound) {
            uart_sendStr("E\tFR\r\n");
            status = BOUNDARY_FRONT_RIGHT;
        }
        else if(l_bound) {
            uart_sendStr("E\tL\r\n");
        ...
        if(fl_hole && fr_hole) {
            ...
            uart_sendStr("H\tFL\r\n");
            status = HOLE_FRONT_LEFT;
        }
        else if(fr_hole) {
            uart_sendStr("H\tFR\r\n");
            status = HOLE_FRONT_RIGHT;
        }
        else if(l_hole) {
            uart_sendStr("H\tL\r\n");
            status = HOLE_LEFT;
